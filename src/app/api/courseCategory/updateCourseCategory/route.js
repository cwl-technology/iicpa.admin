...
export const POST = async (request) => {
    connectDB();
    try {
        const { categoryName, id, categorySlug, title, keywords, metaDescription } = await request.json();
        ...
        const isCategoryExist = await courseCategoryModel.findOne({ categorySlug: formattedSlug, _id: { $ne: id } });
        if (isCategoryExist) {
            return NextResponse.json({ message: "Category already exist", status: 0 });
        }

        const data = await courseCategoryModel.findByIdAndUpdate({ _id: id }, { categoryName, categorySlug: formattedSlug, title, keywords, metaDescription });
        if (!data) {
