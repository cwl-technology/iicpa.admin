...
export const POST = async (request) => {
    connectDB();
    try {
        const { question, answer1, answer2, answer3, answer4, correctAnswer, courseId, chapterId, topicId, subTopicId, id, quizPoints } = await request.json();
        if (!question || !answer1 || !answer2 || !correctAnswer || !id) {
            return NextResponse.json({ message: "Please provide all the fields!", status: 0 });
        }

        const data = await quizModel.findByIdAndUpdate({ _id: id }, { question, answer1, answer2, answer3, answer4, correctAnswer, courseId, chapterId, topicId, subTopicId, quizPoints });
        if (!data) {
