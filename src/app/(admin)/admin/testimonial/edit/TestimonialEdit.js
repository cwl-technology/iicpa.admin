...
const page = () => {
    ...
    const onSubmit = async (data) => {
        try {
            data.ratings = rating;
            data.id = id;
            const res = await axios.post("/api/testimonial/updateTestimonial", data);
    ...
    const getTestimonialData = async () => {
        try {
            const res = await axios.post("/api/testimonial/getTestimonialData", {
                id: id
            });
            if (res.data.status == 1) {
                reset(res.data.data);
                                ...
                                <ul className="nav nav-tabs nav-bordered mb-3 d-flex justify-content-between">
                                    <li className="nav-item">
                                        <a href="#" data-bs-toggle="tab" aria-expanded="false" className="nav-link active p-0">
                                            <h4 className="header-title">Update Testimonial</h4>
                                        </a>
                                    </li>
                                    <li className="nav-item">
