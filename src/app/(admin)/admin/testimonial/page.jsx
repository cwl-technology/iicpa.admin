...
const page = () => {
    ...
    //Permission Logic
    const menuId = "67e638d9a8f1f1d5d22504b2"
    ...
    const getTestimonialsData = async () => {
        try {
            const res = await axios.get(`/api/testimonial/getAllTestimonials`);
    ...
    const handleDelete = async (id) => {
        ...
        if (result.isConfirmed) {
            try {
                const res = await axios.post(`/api/testimonial/deleteTestimonial`, {
                    id: id
                })
                if (res.data.status == 1) {
    ...
    const handleChangeStatus = async (id, status) => {
        ...
        if (result.isConfirmed) {
            try {
                const res = await axios.post(`/api/testimonial/changeStatus`, {
                    id: id,
                    status: status
                })
    ...
    const handlePaginationNext = (e) => {
        e.preventDefault();
        if (pagiEndInd < testimonialData?.length - 1) {
    ...
    useEffect(() => {
        if (testimonialData) {
            let total = testimonialData?.length;
                                                ...
                                                <div className="add-order text-xl-end mt-xl-0 mt-2">
                                                    {
                                                        ...
                                                        <Link href="/admin/testimonial/create" className="btn btn-primary mb-2 me-2"><i className="bi bi-plus-lg"></i> Add Testimonial</Link>
                                                    }
                                                </div>
                                            </div>
                                        </div>
