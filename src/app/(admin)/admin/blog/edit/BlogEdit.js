...
const BlogEdit = () => {
    ...
    const onSubmit = async (data) => {
        try {
            ...
            const res = await axios.post("/api/blog/updateBlog", formdata);
    ...
    const getBlogData = async () => {
        try {
            const res = await axios.post("/api/blog/getBlogById", {
                id: id
            })
            if (res.data.status == 1) {
                reset(res.data.data);
                setBlogImage(res.data.data.image);
                setBlogDesc(res.data.data.blogDescription)
                setExistedBlogImage(res.data.data.blogImage)
            }
        } catch (err) {
            console.log(err);
                                ...
                                <ul className="nav nav-tabs nav-bordered mb-3 d-flex justify-content-between">
                                    <li className="nav-item">
                                        <a href="#" data-bs-toggle="tab" aria-expanded="false" className="nav-link active p-0">
                                            <h4 className="header-title">Update Blog</h4>
                                        </a>
                                    </li>
                                    <li className="nav-item">
                                        ...
                                        <form>
                                            <div className="row">
                                                ...
                                                <div className="col-md-2 d-flex align-items-center">
                                                    {
                                                        blogImage ? <img src={URL.createObjectURL(blogImage)} alt="error" width={"100px"} /> : <img src={`/uploads/${existedBlogImage}`} alt="error" width={"100px"} />
                                                    }
                                                </div>
